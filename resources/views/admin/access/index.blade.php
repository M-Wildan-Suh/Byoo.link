<x-app-layout title="Access">
    <div x-data="auctionTable()"
        ...
        <div class="w-full flex flex-col sm:flex-row gap-2 justify-between items-center">
            <div x-data="{ modalCreate : false }" class="">
                <button
            ...
            <table class="w-full text-sm sm:text-base rounded-md overflow-hidden">
                <thead>
                    <tr class="h-10 bg-byolink-1 text-white divide-x-2 divide-white">
                        <th class=" px-1 sm:px-2 py-1 w-1/4">User</th>
                        <th class=" px-1 sm:px-2 py-1 w-1/4">Web</th>
                        <th class=" px-1 sm:px-2 py-1 w-1/4">Status</th>
                        <th class=" px-1 sm:px-2 py-1 w-1/4">Opsi</th>
                    </tr>
                </thead>
                <tbody>
                    <template x-for="(item, index) in paginatedData" :key="index">
                        <tr :class="index % 2 === 0 ? 'bg-neutral-100' : 'bg-neutral-200'"
                            ...
                            <td class=" px-2 sm:px-4 py-2 text-center font-semibold" x-text="item.user_name"></td>
                            <td class=" px-2 sm:px-4 py-2 text-center" x-text="item.web_title"></td>
                            <td class=" px-2 sm:px-4 py-2 text-center" x-text="item.status"></td>
        ...
        <div x-show="confirmDeleteModal"
            ...
            <div class="w-full max-w-[720px] bg-white pb-6 rounded-md flex flex-col gap-4 relative overflow-hidden border-2 border-byolink-1">
                ...
                <p class="px-6 text-base">Anda akan menghapus Access : <span x-text="modalData.user_name"></span> => <span x-text="modalData.web_title"></span></p>
        ...
        function auctionTable() {
            return {
                ...
                get filteredData() {
                    ...
                    return this.data.filter(item => item.user_name.toLowerCase().includes(this.search.toLowerCase()));
                },
