<!-- Modal -->
<div x-show="gallery" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-20 px-4"
x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0"
x-transition:enter-end="opacity-100" x-transition:leave="transition ease-in duration-200"
x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0">

<!-- Modal Gallery -->
<div @click.away="gallery = false" class="w-full max-w-[720px] bg-white pb-6 rounded-md flex flex-col gap-4 relative overflow-hidden border-2 border-byolink-1">
    <div class=" pt-6 pb-3 bg-byolink-1 text-white z-30">
        <h2 class=" px-6 text-2xl font-bold">Edit Gallery</h2>
    ...
    <div class=" space-y-3 px-6">
        <div class=" w-full">
            <form action="{{route('gallery.title', ['slug' => $slug])}}" method="post">
                @csrf
                <x-admin.component.sectiontitleinput placeholder="Judul Section" value="{{$title ?? ''}}" name="gallery_title" />
            </form>
        </div>
        <div class=" w-full grid grid-cols-3 sm:grid-cols-5 gap-3">
            @foreach ($gallery as $item)
                <div class=" w-full aspect-[3/2] rounded-md overflow-hidden relative">
                    <form action="{{route('gallery.destroy', ['slug' => $slug, 'gallery' => $item->id])}}" class=" w-full h-full" method="post">
                        @csrf
                        @method('DELETE')
                        <img src="{{ asset('storage/images/gallery/' . $item->image) }}" class=" w-full h-full object-cover" alt="{{$item->image_alt}}" srcset="">
            ...
            <div class=" hidden">
                <form 
                    x-data @submit.prevent="submitForm" x-ref="form" action="{{ route('gallery.store', ['slug' => $slug]) }}" method="post" enctype="multipart/form-data">
                    @csrf
                    <input type="file" id="image_gallery" name="image_gallery" class="hidden" @change="$refs.form.submit()">
                </form>
