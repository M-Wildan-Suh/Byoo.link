    ...
    <div x-data="Table()"
        ...
        <div class="w-full flex flex-col sm:flex-row gap-2 justify-between items-center">
            ...
            <div class=" w-full sm:w-auto flex flex-row font-semibold duration-300">
                <input type="text" x-model="search" placeholder="Cari Template..."
            ...
            <table class="w-full text-sm sm:text-base rounded-md overflow-hidden">
                <thead>
                    <tr class="h-10 bg-byolink-1 text-white divide-x-2 divide-white">
                        <th class=" px-1 sm:px-2 py-1 w-1/3">Voucher</th>
                        <th class=" px-1 sm:px-2 py-1 w-1/3">Template</th>
                ...
                <tbody>
                    <template x-for="(item, index) in paginatedData" :key="index">
                        <tr :class="index % 2 === 0 ? 'bg-neutral-100' : 'bg-neutral-200'"
                            ...
                            <td :class="{
                                'text-red-500': item.status === 'used', 
                                'text-green-500': item.status === 'not used'}" 
                                class=" px-2 sm:px-4 py-2 text-center font-semibold" x-text="item.kode"></td>
                            <td class=" px-2 sm:px-4 py-2 text-center" x-text="item.template"></td>
        ...
        <div x-show="confirmDeleteModal"
            ...
            <div
                ...
                <p class="px-6 text-base">Anda akan menghapus voucher : <span x-text="modalData.kode"></span></p>
                <div class="flex justify-end space-x-4 px-6">
                    ...
                    <form :action="`{{ route('voucher.destroy', '') }}/${modalData.id}`" method="POST"
                        class="inline">
                        @csrf
                        @method('DELETE')
                        <button type="submit"
        ...
        function Table() {
            return {
                ...
                get paginatedData() {
                    ...
                    return this.filteredData.slice(start, end).reverse(); // Reverse the filtered data
                },
                ...
                get filteredData() {
                    ...
                    return this.data.filter(item => item.template.toLowerCase().includes(this.search.toLowerCase()));
                },
