...
<html class=" scroll-smooth" lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="font-sans antialiased">
        <div x-data="{ admin: {{ $admin ? 'true' : 'false'}} }" style="background-color: {{$data->templatecolor->bg_color ?? 'white'}}" class=" w-full max-w-[420px] min-h-screen mx-auto sm:my-8 rounded-lg shadow-lg shadow-black/35">
            {{-- Navigation --}}
            @include('components.guest.component.navigation')
            @if ($admin)
                @include('components.guest.component.adminmode')
            @endif
            {{ $slot }}
            {{-- WhatsApp Telephone --}}
            <x-guest.component.footer slug="{{$data->url}}" :background="$data->templatecolor->main_color ?? 'black'" :phonecolor="$data->templatecolor->third_color ?? '#1d588d'" :wacolor="$data->templatecolor->fourth_color ?? '#25D366'" notlp="{{$data->contact->no_tlp ?? ''}}" nowa="{{$data->contact->no_wa ?? ''}}" class=""/>
        </div>
    </body>
