    ...
    <div class=" fixed top-0 left-0 grid grid-col-3 w-full bg-white/80 backdrop-blur-md px-4 md:px-8 py-4 z-10">
        <div class=" w-full max-w-[1080px] mx-auto flex items-center justify-between">
            ...
            <div class=" hidden md:flex flex-row gap-6 items-center text-neutral-500">
                <x-guest.nav-button route="{{route('home')}}" active="{{request()->routeIs('home')}}">Home</x-guest.nav-button>
                <x-guest.nav-button route="{{route('template')}}" active="{{request()->routeIs('template')}}">Template</x-guest.nav-button>
                <x-guest.nav-button route="{{route('home')}}" active="{{request()->routeIs('')}}">Tutorial</x-guest.nav-button>
                @if (Route::has('login'))
                    @auth
                        <x-guest.nav-button route="{{route('my.web')}}" active="{{request()->routeIs('my.web')}}">My Web</x-guest.nav-button>
